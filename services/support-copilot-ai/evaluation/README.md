...
历史边界：干净提交 `59903a1e5fad74cf2b792263f735dafe36b8066c` 是当时 Responses 路径的一次端到端成功；`be9ac60` 和 `b856019` 都是 Responses 4-case dataset 失败，结果均为 1 success + 3 `invalid_model_response` fallbacks，后者人工 review 0/4。`bfb7eee6adae0556399e56457eeed19a158c1d39` 的 `chat_completions` attempt-3 主链路为 `live/SUCCEEDED`（`VECTOR`、3 chunks、1 citation、587/343 tokens、12048 ms、trace 保留），同次 4-case 则为 1 success、3 个 `invalid_model_response` fallback、3/4 retrieval/citation、8533/11415 ms、0/4 `NOT_REVIEWED`、`publishable=false`、`machine-gate-failed`。这是一次失败的机器质量门禁，不是 RAG、dataset 或 publishable success；未授权新的 live rerun、未做人审，Docker deferred。verifier 的绑定不能证明供应商实际路由、token 或 latency 的真实性，也不能独立证明 provider URL、HTTP status、调用次数或 dirty 文件具体内容。
