...
- Trend buckets use seven explicit `[start, end)` UTC `Instant` predicates in [OperationalMetrics.java:49](../../../services/support-copilot-api/src/main/java/com/cyagent/supportcopilot/metrics/OperationalMetrics.java), rather than database `year`/`month`/`day` extraction. This makes bucket membership independent of the database session timezone.
- The MySQL fixture creates the UTC-midnight test entities with their intended `createdAt` before the first persistence operation, respecting the immutable column mapping, in [OperationalMetricsMySqlIntegrationTests.java:30](../../../services/support-copilot-api/src/test/java/com/cyagent/supportcopilot/metrics/OperationalMetricsMySqlIntegrationTests.java).
- Browser clients can read the matching count: `X-Total-Count` is exposed by [WebConfig.java:16](../../../services/support-copilot-api/src/main/java/com/cyagent/supportcopilot/config/WebConfig.java) and checked by a real cross-origin MVC request in [TicketQueueIntegrationTests.java:36](../../../services/support-copilot-api/src/test/java/com/cyagent/supportcopilot/ticket/TicketQueueIntegrationTests.java).
- Persisted duration is asserted against the analyzed response in [AnalysisPersistenceServiceTests.java:67](../../../services/support-copilot-api/src/test/java/com/cyagent/supportcopilot/analysis/AnalysisPersistenceServiceTests.java), and the `IN_PROGRESS -> RESOLVED` transition now verifies the persisted resolved timestamp in [TicketUpdateContractTests.java:293](../../../services/support-copilot-api/src/test/java/com/cyagent/supportcopilot/ticket/TicketUpdateContractTests.java).
