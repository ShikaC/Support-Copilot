...
排序诊断把方向限制得很死：召回已经够用，改进空间在排序与上下文组装。

1. **重排与融合（最高性价比）**：gold 排在第 4–6 名，且前几名分数高度密集（例如 0.8146 / 0.8125 / 0.7975）。先用 `query-vectors.json` 做**零调用**的离线比较定下融合方案（BM25 + RRF 权重、字段加权），确定值得后才引入真实重排模型。
2. **query 构造实验**：当前 query 是 `subject + 整段多轮对话 + 固定评估前缀`，真实问题只是最后一句用户话。在同批题目与同批缓存向量上比较「只取末尾用户轮次」「去掉评估前缀」等变体（需新调用，量级与本次相同）。
3. **切片方式实验**：未命中的 3 题 gold 只有 1–2 个片段，而 8 个片段的题排第 1。固定 query 与向量、只变切片，验证这个相关性观察。
4. **离线参数实验**：用 `query-vectors.json` 重跑 top_k、阈值、融合与重排，零调用。注意 `top_k` 是成本决策，不要记作质量改进。
5. 待 chat 端点恢复后执行预注册的 development 比较，得到「检索 + 生成 + 引用」的完整对照。
