...
按领域：dmv 3/3（100%）、studentaid 2/2（100%）、ssa 2/3（66.7%）、va 1/3（33.3%）。`va` 是最弱领域，也是下一步最值得看的切片。
...
1. **query 构造的进一步实验**：当前 query 是 `subject + 整段多轮对话 + 固定评估前缀`，真实问题只是最后一句用户话。可在同一批题目与同一批缓存向量上比较「只取末尾用户轮次」「去掉评估前缀」等变体——需要新调用，但量级与本次相同。
2. **检查 `va` 的 2 个未命中**：区分是切片问题、query 稀释，还是 gold 文档确实不含答案。
3. **离线参数实验**：用 `query-vectors.json` 重跑 top_k、阈值、BM25 融合与重排，零调用。
4. 待 chat 端点恢复后执行预注册的 development 比较，得到「检索 + 生成 + 引用」的完整对照。
