...
> 更新时间：2026-09-19
> 状态分类：语料生成任务 B1 已实现并完成本地验收；最终提交与远端结果以最新交付核对。索引重建 A 的最终边界 `444bbde` 已推送且四条 CI 通过；该 CI 证据不覆盖 B1。

## 最新：语料生成任务 B1（2026-09-19）

用户要求继续后，沿用推荐的 Python 调 Node 方案，实现预配置 Doc2Dial 文档到候选语料的后台任务。共享原有切片逻辑，固定源 bytes 和摘要，独立默认关闭开关，内部鉴权，持久化任务，跨进程锁与期限。成功只产出候选文件；没有修改评估集、调用模型、生成索引或激活当前检索。

本机真实 HTTP 与重启验收、冻结语料/映射逐字节对拍已通过。对抗式审查发现父服务被杀后 Node 脱离锁的问题，已增加继承锁和 Node 自身期限，复现与回归证据统一见 [B1 报告](verification/corpus-build-tasks-2026-09-19/README.md)。最终回归、提交和远端状态以该报告与最新交付核对。

本地源码运行需要 Node；当前 AI Docker 镜像未包含 Node 和固定脚本，B1 尚不支持该镜像。下一步是审阅 B1 diff 后，处理已成功候选语料与索引构建之间的显式交接；Java/前端、上传与部署仍各自独立。没有新增作者已理解或真人质量审核结论。

## 已完成：索引重建任务化切片 A（2026-09-19）

已实现内部 POST 接受构建、GET 查询持久化任务；线程执行、同 artifact root 文件锁、分批真实进度、语料摘要和调用预算检查、失败现场保留及重启查询。成功不自动激活或 reload，原工单分析/检索快照保持原行为。完整 Python 回归与本机 Uvicorn + 合成 Embedding HTTP 验收通过，详细命令、数量、故障证据和限制统一见[切片 A 报告](verification/index-rebuild-tasks-2026-09-19/README.md)。

本轮没有真实供应方费用，也未使用 holdout。写功能默认关闭；显式启用并提供真实供应方配置后会产生真实 Embedding 调用，mock 分析模式不替代重建预算。该切片未接 Java/前端入口或上传解析；后续语料生成进展见上方 B1。未新增作者已理解或人工质量审核结论。

## 已完成：推送前对抗式审查（2026-09-18 至 09-19）

用户已明确授权修复审查问题后推送 GitHub。本轮修复索引 reload 意外付费构建、并发快照回退与错误路径泄漏；检索实验重复消费、来源混用与非有限向量；已保存分析的知识权限绕过及跨调用者幂等回放；并完成 HTTPX2、Tomcat、Vitest 安全补丁升级。当前已确认问题均修复，有界复查没有剩余阻断项；验证命令、结果、兼容变化、八个本地提交和限制统一见[本轮审查记录](verification/adversarial-review-2026-09-18/README.md)。本地通过不等于远端 CI、生产部署或绝对无漏洞。

`093cd52` 首次推送的四条 GitHub CI 因遗漏测试 fixture 和 runner 缺少工具失败。后续修复这些依赖、GNU tar 参数顺序、Gitleaks 安装和权限设置；最终 `1de4a77` 的 Python、Java、React、非容器发布门禁四条远端工作流均通过，详见审查记录与该提交 Actions。此结论不自动覆盖之后的重建任务切片。

当前评测 CLI 要求执行时计划与 query 向量摘要。下文 09-17/09-18 缓存不含该取证信息，继续保留历史报告，但不能用当前工具直接重算或事后补哈希冒充新验证。新实验需要新输出目录、独立执行 claim 与明确预算；不自动重跑旧批次。

## 历史实验：评估集扩容到 76 题，融合实验在 40 题上重测（2026-09-18）

以下保留实验当时的事实：针对「11 道可评估题无法验证任何检索改动」这一卡点，把 development 从 11 道扩到 40 道，并在新集上重跑融合实验。该实验消耗 40 次 embedding 调用、0 次生成调用。本轮审查没有新增这些调用。
