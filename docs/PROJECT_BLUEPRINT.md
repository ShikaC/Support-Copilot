...
项目当前虽然仍处于 V1，但最终目标不是停留在演示原型，而是持续演进为一个企业级、可运行、可使用、可验证的成熟 AI 应用项目，并形成能够用于 AI 应用开发岗位求职的真实项目经历。

这里的“企业级”指项目最终需要具备完整业务闭环、稳定契约、持久化、安全、审计、错误处理、测试、CI、可观测性、配置隔离和部署等工程能力。它不代表项目已经在某家真实企业的生产环境中运行；简历仍然只能使用代码和验证证据支持的事实。
...
- `OPENAI_API_KEY`
- `OPENAI_EMBEDDING_API_KEY`，可选的独立 Embedding 服务凭据；未配置时回退到 `OPENAI_API_KEY`
- `OPENAI_BASE_URL`，聊天/Responses 请求地址，仅在使用兼容网关时配置
- `OPENAI_EMBEDDING_BASE_URL`，Embedding 请求地址；未配置时回退到 `OPENAI_BASE_URL`
- `OPENAI_CHAT_MODEL`
...
AI_PROCESSING_TIMEOUT_SECONDS=90
...
| 无法解释代码 | 能运行但面试讲不清 | 每完成模块进行代码审查和完整讲解说明 |
