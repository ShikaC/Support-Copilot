...
- focused profile/migration tests：5 passed，0 failed，0 skipped。
- Java 全量非容器 tests：93 total，90 passed，0 failed，3 skipped；三个 skip 全部属于 Task 15 的 `MySqlProfileIntegrationTests`，不计为 Task 3 通过。
- `compileTestJava`：通过，包含 MySQL integration 与 stale-schema 测试源码。
- 手动 demo：健康状态 `UP`，8 条工单、7 条 seeded analyses，H2 Console HTTP 302；进程已退出且端口 18182 已释放。
- 手动 pilot 缺配置：exit 1，明确命名 `SUPPORT_COPILOT_DB_URL`，日志无 H2 fallback、H2 文件集合未变化；端口 18183 已释放。
