...
取消负责人和其他工单 API 已受 JWT 角色门禁保护，审核与审计 actor 来自同一个 trusted actor provider；`demo` 仍保留明确标记的匿名演示身份。当前审计覆盖工单、分析持久化和审核动作；Task 8 才接入知识发布，Task 15 才验证 MySQL parity。React 登录/token adapter 和真实 OIDC 联调仍未实现，不能把当前合成 decoder/H2 证据描述成生产身份或合规审计系统。
...
第 57 轮增加 `scripts/run-local-smoke.sh`，使用隔离端口启动 mock Python、Java 和 React，依次执行健康检查、跨服务成功分析和前端运行时契约门禁，并在结束后清理自己启动的进程。该脚本不调用正式 API，也不会停止端口已被占用的已有服务。

第 58 轮将 mock 评估集扩充到 31 条，增加低置信度、近邻、无证据和隐私组合请求，并为 mock 与 live 检索分别增加可配置最低分数门禁。测试验证低分向量不会进入证据列表、无证据时不会调用模型；同时修正隐私规则优先级，避免数据导出删除请求被普通导出规则抢先匹配。当前 mock 评估 31/31 通过，真实 live 阈值仍需使用脱敏分数分布校准。
