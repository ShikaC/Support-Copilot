...
export function OverviewView({ metrics, tickets }: { readonly metrics: Metrics | null; readonly tickets: readonly Ticket[] }) {
  ...
  return <div className="view-enter"><StatusStrip metrics={metrics} /><div className="dashboard-grid">
    <div><section className="dashboard-panel"><div className="panel-header"><div className="panel-heading"><h2 className="panel-title">工单趋势</h2><div className="panel-meta">最近 7 天新建与解决数量</div></div><Tag>最近 7 天</Tag></div>
      <div className="chart-panel-body" aria-describedby="ticket-trend-data" data-reduced-motion={reducedMotion}>{metrics.ticketTrend.length > 0 ? <ReactECharts option={trendOption} style={{ height: '100%' }} /> : <UnavailablePanel title="趋势数据暂不可用" description="当前接口只返回工单快照，尚未提供可追溯的历史趋势数据。" />}</div>
      <div className="visually-hidden" id="ticket-trend-data"><table aria-label="工单趋势数据"><thead><tr><th>日期</th><th>新建</th><th>解决</th></tr></thead><tbody>{metrics.ticketTrend.map((item) => <tr key={item.date}><th>{item.date}</th><td>{item.created}</td><td>{item.resolved}</td></tr>)}</tbody></table></div></section>
