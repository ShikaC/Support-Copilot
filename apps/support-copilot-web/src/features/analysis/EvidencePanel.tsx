...
export function EvidencePanel({ analysis }: { readonly analysis: AnalysisResult }) {
  const hasEvidence = analysis.retrieval.hits.some((hit) => hit.usedAsEvidence)
  ...
  return <div className="analysis-content">
    ...
    {!hasEvidence && <div className="no-evidence"><FileSearch size={20} /><strong>没有找到充分证据</strong><span>当前没有可作为回复依据的知识，需要人工复核。</span></div>}
      ...
      <div className="evidence-list">{analysis.retrieval.hits.map((hit) => {
        ...
        return <article className="evidence-item" key={hit.chunkId}>
          <button className="evidence-button" type="button" aria-expanded={expanded} onClick={() => setExpandedId(expanded ? null : hit.chunkId)}>
            <span className="evidence-rank">{hit.rerankPosition}</span><span><span className="evidence-title">{hit.documentTitle}</span><span className="evidence-section">{hit.section} · {hit.usedAsEvidence ? '已作为回复依据' : '未作为回复依据'}</span></span><span className="evidence-score">{(hit.rerankScore * 100).toFixed(1)}</span>{expanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
          </button>
          <div className={`evidence-expanded ${expanded ? 'open' : ''}`}><div className="evidence-expanded-inner"><p className="evidence-copy">{hit.content}</p><div className="evidence-meta"><span>{hit.retrievalMethod}</span></div></div></div>
        </article>
      })}</div>}
  </div>
}
